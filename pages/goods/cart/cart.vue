                ...
                <view class="goods-info">
                  ...
                  <view class="sku" v-if="item.sku_spec_format">
                    <view class="goods-spec">
                      <block v-for="(x, i) in item.sku_spec_format" :key="i" v-if="item.luminosity_status != 1">{{ x.spec_value_name }} {{ i < item.sku_spec_format.length - 1 ? ';' : '' }}</block>
                    </view>
                  </view>
...
.cart-bottom {
  ...
  // bottom: 56rpx;
  z-index: 9;
  bottom: calc(constant(safe-area-inset-bottom) + 135rpx);
  bottom: calc(env(safe-area-inset-bottom) + 135rpx);
