  ...
  <view class="order-container" :class="{ 'safe-area': isIphoneX }" :data-theme="themeStyle">
        ...
        <view class="info-wrap" v-if="orderPaymentData.member_address" @click="selectAddress">
          ...
          <view class="content">
            ...
            <view class="desc-wrap">
              {{ orderPaymentData.member_address.full_address ? orderPaymentData.member_address.full_address : '' }} {{ orderPaymentData.member_address.address ? orderPaymentData.member_address.address : '' }}
            </view>
          </view>
        </view>
    ...
    <view class="site-wrap" v-for="(siteItem, siteIndex) in orderPaymentData.shop_goods_list" :key="siteIndex">
      ...
      <view class="site-body">
        <!-- 商品 -->
        <view class="goods-wrap" v-for="(goodsItem, goodsIndex) in siteItem.goods_list" :key="goodsIndex" style="border-bottom: 2rpx solid #F1F1F1;">
          ...
          <view class="goods-info">
            <view @click="navigateTo(goodsItem.sku_id)" class="goods-name" style="font-size: 35rpx;font-weight: bold;" v-if="goodsItem.luminosity_status != 1">{{ goodsItem.sku_name }}</view>
            <sku-list :goodsItem="goodsItem" :machining_pupil="JSON.parse(orderPaymentData.machining_pupil || '{}') || {}"></sku-list :item="goodsItem">
 <!--           <view class="sku" v-if="goodsItem.sku_spec_format">
      ...
      <view class="site-footer">
        ...
        <view class="order-cell liuyanBox" v-if="orderCreateData.buyer_message">
          ...
          <view class="liuyan"><textarea class="myliuyan" type="text" placeholder="留言前建议先与商家协调一致" placeholder-class="color-tip" v-model="orderCreateData.buyer_message[siteItem.site_id]" /></view>
        </view>
    ...
    <view class="order-money">
      ...
      <view class="order-cell">
        <text class="tit">加工费</text>
       <view class="box color-title">
          <text class="unit">{{ $lang('common.currencySymbol') }}</text>
          <text class="money">{{ orderPaymentData.machining_money | moneyFormat }}</text>
        </view>
      </view>
      <view class="order-cell" v-if="orderPaymentData.delivery_money > 0">
          ...
          <view>
            ...
            <block v-if="orderInvoice.is_invoice == 1">
              ...
              <view class="invoice-cell">
                ...
                <view class="option-grpup">
                  <view class="option-item" :class="{ 'color-base-bg active': orderInvoice.invoice_title_type == 1 }" @click="changeInvoiceTitleType(1)">个人</view>
                  <view class="option-item" :class="{ 'color-base-bg active': orderInvoice.invoice_title_type == 2 }" @click="changeInvoiceTitleType(2)">企业</view>
                </view>
              </view>
              <view class="invoice-cell">
    ...
    <uni-popup ref="presentCouponPopup" type="bottom">
      <view class="coupon-popup popup" @touchmove.prevent.stop>
        ...
        <view class="popup-footer" :class="{ 'bottom-safe-area': isIphoneX }"><view class="confirm-btn color-base-bg" @click="closePopup('presentCouponPopup')">确定</view></view>
      </view>
    </uni-popup>
    ...
    <uni-popup ref="couponPopup" type="bottom">
      <view class="coupon-popup popup" @touchmove.prevent.stop>
        ...
        <view class="popup-footer" :class="{ 'bottom-safe-area': isIphoneX }"><view class="confirm-btn color-base-bg" @click="popupConfirm('couponPopup', siteCoupon.site_id)">确定</view></view>
      </view>
    </uni-popup>
    ...
    <uni-popup ref="PlatcouponPopup" type="bottom">
      <view class="coupon-popup popup" @touchmove.prevent.stop>
        ...
        <view class="popup-footer" :class="{ 'bottom-safe-area': isIphoneX }"><view class="confirm-btn color-base-bg" @click="popupConfirm('PlatcouponPopup')">确定</view></view>
      </view>
    </uni-popup>
    ...
    <uni-popup ref="sitePromotionPopup" type="bottom">
      <view class="promotion-popup popup">
        ...
        <view class="popup-footer" :class="{ 'bottom-safe-area': isIphoneX }"><view class="confirm-btn color-base-bg" @click="closePopup('sitePromotionPopup')">确定</view></view>
      </view>
    </uni-popup>
    ...
    <uni-popup ref="deliveryPopup" type="bottom">
      <view class="delivery-popup popup" @touchmove.prevent.stop>
        ...
        <view class="popup-body" :class="{ 'safe-area': isIphoneX }">
          ...
          <block v-for="(deliveryItem, deliveryIndex) in siteDelivery.data" :key="deliveryIndex" v-if="orderPaymentData.delivery[siteDelivery.site_id].delivery_type == deliveryItem.name">
                  ...
                  <view class="delivery-content">
                    <view class="item-wrap" v-for="(item, index) in deliveryItem.store_list" :key="index" @click="selectPickupPoint(item.store_id)">
                      <view class="detail">
                        ...
                        <view class="info">
                          <view :class="item.store_id == orderPaymentData.delivery[siteDelivery.site_id].store_id ? 'color-base-text' : ''" class="font-size-goods-tag">营业时间：{{ item.open_date }}</view>
                          <view :class="item.store_id == orderPaymentData.delivery[siteDelivery.site_id].store_id ? 'color-base-text' : ''" class="font-size-goods-tag">
                            地址：{{ item.full_address }}{{ item.address }}
                          </view>
                        </view>
                      </view>
                      <view class="icon" v-if="item.store_id == orderPaymentData.delivery[siteDelivery.site_id].store_id"><text class="iconfont iconyuan_checked color-base-text"></text></view>
                    </view>
            ...
            <view class="local-box" v-show="orderPaymentData.delivery[siteDelivery.site_id].delivery_type == 'local' && orderPaymentData.delivery[siteDelivery.site_id].showTimeBar">
              <view class="order-cell">
              ...
              <picker
                v-if="orderPaymentData.delivery[siteDelivery.site_id].showTimeBar && orderPaymentData.member_address && orderPaymentData.delivery[siteDelivery.site_id].canLocalDelicery"
        ...
        <view class="popup-footer" :class="{ 'bottom-safe-area': isIphoneX }"><view class="confirm-btn color-base-bg" @click="closePopup('deliveryPopup')">确定</view></view>
      </view>
    </uni-popup>
