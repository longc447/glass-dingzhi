export default {
  ...
  methods: {
          ...
          if (res.code >= 0) {
            ...
            for (var index in res.data.shop_goods_list) {
              if (!res.data.shop_goods_list[index].present_list) {
                ...
                res.data.shop_goods_list[index].goods_list = res.data.shop_goods_list[index].goods_list.concat(res
            ...
            console.dir(this.orderPaymentData, "商品数据111111111111");
    ...
    handlePaymentData() {
            ...
            if (siteItem.local_config.info.time_type == 0) {
              ...
              if (siteItem.local_config.info.time_week.length == 7) {
                ...
                if (is_interval) {
                  ...
                  this.orderCreateData.delivery[key].deliveryWeek = weeks[timeWeek[0]] + "至" + weeks[timeWeek[
                    timeWeek.length -
                    1]];
                }
              }
            ...
            if (this.orderCreateData.delivery[key].canLocalDelicery) {
              ...
              if (!(start_hour == end_hour && start_minute == end_minute)) {
                ...
                if ((hour < start_hour) || (hour == start_hour && minute < start_minute)) {
                  this.orderCreateData.delivery[key].buyer_ask_delivery_time = (start_hour.toString().length == 1 ?
                      "0" +
                      start_hour :
                      start_hour) + ':' +
      ...
      // this.orderPaymentData.shop_goods_list = res?.data?.data?.shop_goods_list.map(e => {
      //   return {
      //     ...e,
      //     goods_list: e.map(i => {
      //       return {
      //         ...i,
      //         remarks: typeof i.remarks == 'string' ? JSON.stringify(i.remarks) : i.remarks
      //       }
      //     })
      //   }
      // })
        ...
        success: res => {
          if (res.code >= 0) {
            ...
            Object.keys(res.data.shop_goods_list).forEach((key, index) => {
              ...
              this.orderPaymentData.shop_goods_list[key].invoice_delivery_money = siteItem
    ...
    verify() {
      ...
      if (this.orderPaymentData.is_virtual == 0) {
        ...
        for (let key in this.orderCreateData.delivery) {
          ...
          if (this.orderCreateData.delivery[key].delivery_type == 'store' && this.orderCreateData.delivery[key]
            .store_id ==
            0) {
            deliveryVerify = false;
            this.$util.showToast({
          ...
          if (this.orderCreateData.delivery[key].delivery_type == 'local') {
            ...
            if (this.orderCreateData.delivery[key].canLocalDelicery) {
              ...
              if (!(start_hour == end_hour && start_minute == end_minute)) {
                ...
                if (!((hour > start_hour && hour < end_hour) || (hour == start_hour && minute > start_minute) || (
                    hour ==
                    start_hour && minute >= start_minute && hour < end_hour))) {
                  this.$util.showToast({
    ...
    imgError(siteIndex, goodsIndex) {
      this.orderPaymentData.shop_goods_list[siteIndex].goods_list[goodsIndex].sku_image = this.$util.getDefaultImage()
        .default_goods_img;
      this.$forceUpdate();
    },
    // 购物车数量
    ...
    openInvoicePopup(val) {
      ...
      this.orderInvoice.invoice_title_type = this.orderInvoice.invoice_title_type == undefined ? 1 : this.orderInvoice
        .invoice_title_type;
  ...
  computed: {
    // 余额抵扣
    balanceDeduct() {
      if (this.orderPaymentData.member_account.balance_total <= parseFloat(this.orderPaymentData.order_money).toFixed(
          2)) {
