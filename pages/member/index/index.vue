    ...
    <view class="container" style="padding-bottom: 200rpx;">
      <view class="member-container">
          ...
          <view class="user-section-box">
            <view class="user-info-box" v-if="token">
              <view @click="$util.redirectTo('/pages/member/info/info')" class="user-head">
                <image :src="memberInfo.headimg ? $util.img(memberInfo.headimg) : $util.getDefaultImage().default_headimg" mode="aspectFill" @error="memberInfo.headimg = $util.getDefaultImage().default_headimg"></image>
              </view>
              <view class="user-box">
                ...
                <view style="display: flex;">
                  ...
                  <view v-if="is_wholesaler == 3" class="user-label" style="margin-left: 10rpx;"><text>批发商</text></view>
                </view>
              </view>
            </view>
            ...
            <view @click="redirectToLink('/pages/member/info/info')" class="user-set" v-if="token"><view :style="defaultTextColor" class="iconfont iconshezhi"></view></view>
        ...
        <view class="member-body">
          ...
          <view v-if="defaultInfo.level == 1" class="member-level" :class="{ 'no-default-level': defaultInfo.menuStyle != 'palace' && defaultInfo.topStyle != 'default' }" @click="jumpLevel()">
            <view class="member"><image :src="$util.img('upload/uniapp/member/index/member.png')" mode="aspectFit"></image></view>
          ...
          <view class="example-body" :style="{ background: defaultInfo.topStyle == 'default' || defaultInfo.menuStyle == 'palace' ? '#fff' : '#f7f7f7' }">
            <view class="example-body-head" :class="{ 'example-tit-palace': defaultInfo.menuStyle != 'palace' && defaultInfo.topStyle != 'default' }"><text class="example-tit">常用工具</text></view>
              ...
              <uni-grid :column="4" :show-border="false" :square="false">
                <block v-for="(item, index) in defaultInfo.menuList" :key="index">
                  <view @click="redirectToLink(item.url)" v-if="menuIsShow(item)">
                    <block v-if="item.tag == 'servicer'"></block>
                    <block v-else>
                      <uni-grid-item>
            ...
            <view v-if="defaultInfo.menuStyle == 'list'" class="list-style">
              <block v-for="(item, index) in defaultInfo.menuList" :key="index">
                <view class="list-style-item" :class="defaultInfo.insertGap == 0 ? 'no-interval' : 'interval'" @click="redirectToLink(item.url)" v-if="menuIsShow(item)">
